...
highest `treatment size` (amount/quality of exposure) to CLVs.
    ...
    \noindent \textbf{H1:} Leaders who attended more autonomous and
egalitarian institutions (\textbf{Anglo-American}) are more likely to
implement liberal reform.\\

    \noindent \textbf{H2 (Specialization):} Leaders who specialized in the
social sciences or humanities -- \textbf{especially economics or law} -- at
autonomous and egalitarian institution are more likely to implement liberal
reform.

    \subsubsection{Policy Outcomes (Dependent Variables)}

    \noindent Leader policy implementation is measure as the change between
the beginning and end of a leader's tenure across five liberal policy areas:
    \begin{itemize}
        \item Trade liberalization;
        \item Rule of law (Judicial independence);
        \item Financial openness (capital controls intensity);
        \item Human rights protections (physical integrity rights);
        \item Liberal democratization (V-Dem score).
    \end{itemize}

    \subsubsection{Key Findings}

    \noindent \textbf{H1 Supported: Anglo-American education} is associated
with a consistent and statistically significant liberalizing effect across \textit{all} five policy outcomes.
    \begin{itemize}
        \item This effect is qualitatively different and often larger than
        that of Other Western institutions.
    \end{itemize}

    \noindent \textbf{General Education Effect:} All college-educated
leaders, regardless of education model, are more likely to implement liberal
democratic reform than leaders with no college education.\\

    \noindent \textbf{H2 Supported (AA Specialization):} Leaders who studied \textbf{Economics or Law} at Anglo-American institutions are consistently more likely to implement liberal reform across policy areas.
    \begin{itemize}
        \item AA Economics and AA Law coefficients are statistically
        indistinguishable.
        \item This similarity suggests the mechanism is
        \textbf{exposure to broad liberal values} (CLV transmission), not
        just acquisition of technical competencies specific to one field.
    \end{itemize}

    \noindent \textbf{Non-AA Specialization:} Generally not associated with
consistent liberal outcomes, with minor exceptions (e.g., non-AA Law
slightly affecting trade liberalization/rule of law).\\

    \noindent \textbf{Military Schools:} Leaders with military school
backgrounds (AA or non-AA) are generally statistically indistinguishable
from leaders with no tertiary education, suggesting minimal impact from
military socialization. Non-AA military backgrounds are linked to
\textit{rolling back} liberal democracy (negative effect).

    \subsubsection{Endogeneity/Self-Selection}

    \noindent The theory addresses potential self-selection (liberal leaders
choosing AA schools) by noting that illiberal elites frequently send their
children to prestigious AA universities for ranking/language reasons, often
despite having status-quo bias.\\

    \noindent Leaders from elite backgrounds are statistically more likely (
23\%) to attend AA universities than non-elite leaders (15\%).\\

    \noindent Differences in policy effects between AA and Other Western
institutions can be interpreted as causal because admission relies on merit (
grades, scores), decoupling the final school choice from underlying ideology.
